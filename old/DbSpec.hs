...
spec = describe "Db" $ do
  withTestDB $ describe "setup" $ do
    ...
    it "applyMigration/listMigrations returns the version applied" $ withDB $ do
    ...
    it "applyMigrations/getAllMigrationRows" $ withDB $ do
      resetMigrations
      ApplicationRow {..} <- applyMigrations migrationGroup

      -- getAllApplicationRecords works too
      fmap toList getAllApplicationRecords `shouldReturn` [ApplicationGroup
        [ MigrationRow
